...
- All 39 current styles include `summary`, `tags`, `inputType`, and typed `examples` metadata.
...
- Release readiness audit passes: 39 unique styles, 39 official examples, all referenced assets present, ESLint clean, production build clean, and zero dependency vulnerabilities.
...
- `生活速寫混合媒介` was added as an independent single-image style in `手繪插畫`; both Anson-provided references were normalized to complete 9:16 WebP previews without cropping.
- `品牌主視覺企劃` was added as an independent single-image style in `視覺企劃`; both Anson-provided references were normalized to complete 9:16 WebP previews without cropping.
- `笨拙手寫詩意海報` was added as the first independent `text-only` style in `手繪插畫`; it uses only an official 9:16 generation example and never displays a fabricated original.
- Desktop, mobile detail, and mobile full-screen preview now support text-only styles with a `生成範例` label and no comparison controls.
- `晨光城市旅行誌` was added as an independent single-image style in `城市旅遊`; both Anson-provided references were normalized to complete 9:16 WebP previews without destructive cropping.
- `樂高積木微縮世界` was added as an independent single-image style in `手繪插畫`; both Anson-provided references were normalized to complete 9:16 WebP previews without destructive cropping.
- `實景人物貼紙線稿` was added as an independent single-image style in `手繪插畫`; both Anson-provided references were normalized to complete 9:16 WebP previews without destructive cropping.
- `斑駁光影電影感` was added as an independent single-image style in `電影宇宙`; both Anson-provided references were normalized to complete 9:16 WebP previews without destructive cropping.
- `午後窗光輕食時光` was added as an independent single-image style in `食物美學`; both Anson-provided references were normalized to complete 9:16 WebP previews without destructive cropping.
